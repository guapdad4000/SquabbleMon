...
content = content.replace(
  /if \(window\.originalPlayerClass\) {[\s\S]*?playerSprite\.style\.animation = "float 2s ease-in-out infinite";[\s\S]*?}/g,
  'setPlayerAnimation("default");',
);

// 2. Replace opponent animation restoration code in various places
content = content.replace(
  /if \(window\.originalOpponentClass\) {[\s\S]*?opponentSprite\.style\.animation = "float 2s ease-in-out infinite";[\s\S]*?opponentSprite\.style\.transform = "scaleX\(-1\)";[\s\S]*?}/g,
  'setOpponentAnimation("default");',
);

// 3. Fix animation code in hitSelf function
const hitSelfPattern =
  /playerSprite\.style\.animation = "player-shake-animation 0.5s ease-in-out";[\s\S]*?setTimeout\(\(\) => {[\s\S]*?playerSprite\.style\.animation = "float 2s ease-in-out infinite";[\s\S]*?}, 500\);/g;
content = content.replace(
  hitSelfPattern,
...
const opponentShakePattern =
  /opponentSprite\.style\.animation = "shake-animation 0.5s ease-in-out";[\s\S]*?setTimeout\(\(\) => {[\s\S]*?opponentSprite\.style\.animation = "float 2s ease-in-out infinite";[\s\S]*?opponentSprite\.style\.transform = "scaleX\(-1\)";[\s\S]*?}, 500\);/g;
content = content.replace(
  opponentShakePattern,
