...
(function() {
  ...
  function initFix() {
    ...
    try {
      ...
      characterSprite = document.querySelector('#overworld-canvas .player-sprite');
      if (characterSprite) {
  ...
  function handleEmergencyKeydown(event) {
    try {
      ...
      const isInDialogue = dialogueBox && 
      ...
      if (newY >= 0 && newY < currentMap.length && 
        ...
        if (tileType === TILE_TYPES.WALKABLE && !npcAtPosition) {
          ...
          if (tileType === TILE_TYPES.GRASS) {
            ...
            if (encounterChance < 0.1) { // 10% chance
  ...
  function updatePlayerPositionVisual() {
    try {
      if (!player || !characterSprite) {
        log("Cannot update player position visually: missing player object or characterSprite");
        return;
      }
      ...
      log(`Updated player position visual to (${player.x}, ${player.y}) facing ${player.direction}`);
    } catch (error) {
  ...
  function getTileFacingPlayerEmergency() {
    try {
      ...
      if (facingY >= 0 && facingY < currentMap.length && 
          facingX >= 0 && facingX < currentMap[0].length) {
        return { 
          x: facingX, 
          y: facingY, 
  ...
  function interactWithFacingTileEmergency() {
    try {
      ...
      log(`Interacting with tile at (${facingTile.x}, ${facingTile.y}) of type ${facingTile.type}`);
      
      // Check for NPC
      if (currentNpcs) {
        const npcAtTile = currentNpcs.find(npc => 
          npc.x === facingTile.x && npc.y === facingTile.y);
      ...
      if (facingTile.type === TILE_TYPES.DOOR && currentDoors) {
        const doorAtTile = currentDoors.find(door => 
          door.x === facingTile.x && door.y === facingTile.y);
        
        if (doorAtTile) {
          log(`Found door at tile leading to ${doorAtTile.leadsTo}`);
          changeZoneEmergency(doorAtTile.leadsTo, doorAtTile.entranceX, doorAtTile.entranceY);
          return;
        }
      }
  ...
  function startDialogueEmergency(npc) {
    try {
      ...
      // Update name element
      const nameElement = document.getElementById('dialogue-name') || document.getElementById('npc-name');
      if (nameElement) {
  ...
  function advanceDialogueEmergency() {
    try {
      ...
      if (!window.currentDialogue || typeof window.currentDialogueLine === 'undefined') {
      ...
      if (window.currentDialogueLine >= window.currentDialogue.lines.length) {
        ...
        if (window.currentDialogue.opensShop && typeof window.openShop === 'function') {
      ...
      const textElement = document.getElementById('dialogue-text');
      if (textElement) {
        textElement.textContent = window.currentDialogue.lines[window.currentDialogueLine];
      }
          ...
          npcElement.addEventListener('click', function() {
            ...
            if (currentNpcs) {
              ...
              if (match) {
                ...
                const npcData = currentNpcs.find(npc => npc.x === x && npc.y === y);
                if (npcData) {
