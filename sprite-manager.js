...
const NINJA_CHARACTER_SPRITES = {
  ...
  left: ["public/sprites/left 1.png", "public/sprites/left 2.png"],
  right: ["public/sprites/right 1.png", "public/sprites/right 2.png"],
};
...
function updatePlayerSprite(
  ...
  if (!inBattle) {
    console.log(
      "Using standard overworld player sprite rather than character-specific sprite",
    );
...
function getGameSpriteUrl(spritePath) {
  ...
  if (lowercasePath.includes("og_ras") || lowercasePath.includes("rasta")) {
    ...
    lowercasePath.includes("brick") ||
    lowercasePath.includes("fitness") ||
    lowercasePath.includes("workout")
  ) {
    return "https://i.imgur.com/YeMI4sr.png";
  } else if (
    lowercasePath.includes("tech") ||
    lowercasePath.includes("computer")
  ) {
    ...
    lowercasePath.includes("momma") ||
    lowercasePath.includes("kitchen")
  ) {
...
function debugSpriteIssues(context, character) {
  ...
  console.log(
    "Character:",
    character ? character.name : "No character provided",
  );
  ...
  if (character) {
    ...
    if (character.sprite) {
      ...
      if (character.name) {
        console.log(
          "Attempting to assign default sprite based on character name...",
        );
  ...
  const playerSpriteContainer = document.getElementById(
  ...
  if (playerSpriteContainer) {
    ...
    console.log(
      "  Dimensions:",
      playerSpriteContainer.style.width,
      "x",
      playerSpriteContainer.style.height,
    );
    ...
    if (playerSpriteElement) {
      ...
      console.log(
        "  Dimensions:",
        playerSpriteElement.style.width,
        "x",
        playerSpriteElement.style.height,
      );
      console.log("  Z-index:", playerSpriteElement.style.zIndex);
    } else {
      console.warn(
        "⚠️ Neither player sprite container nor element found in DOM",
      );
    }
  }
