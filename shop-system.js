...
function getTrapItems() {
  return [
    ...
    },
    {
      ...
      price: 100,
      description:
        "Boosts Attack by 50% but reduces Defense by 20% for 4 turns",
      icon: "⚡",
      stock: 2,
    },
    {
...
function buyItem(shop, itemId, playerInventory, quantity = 1) {
  if (!shop || !itemId || !playerInventory)
...
function sellItem(shop, itemId, playerInventory, quantity = 1) {
  ...
  // Check regular items
  const itemIndex = playerInventory.items.findIndex(
    (item) => item.id === itemId,
  );
  if (itemIndex !== -1) {
  ...
  if (!itemToSell && playerInventory.moveChips) {
    const chipIndex = playerInventory.moveChips.findIndex(
      (chip) => chip.id === itemId,
    );
    if (chipIndex !== -1) {
  ...
  if (!itemToSell && playerInventory.gear) {
    const gearIndex = playerInventory.gear.findIndex(
      (gear) => gear.id === itemId,
    );
    if (gearIndex !== -1) {
  ...
  if (itemCategory === "items") {
    window.InventorySystem.removeItemFromInventory(
      playerInventory,
      itemId,
      quantity,
    );
    ...
    const index = playerInventory[itemCategory].findIndex(
      (i) => i.id === itemId,
    );
    if (index !== -1) {
