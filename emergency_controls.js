...
(function() {
  ...
  function init() {
    ...
    try {
      // Find player element
      playerElement = document.querySelector('.player-sprite') || 
  ...
  function updatePlayerSprite() {
    ...
    if (window.SpriteManager && typeof window.SpriteManager.updatePlayerSprite === 'function') {
      try {
        window.SpriteManager.updatePlayerSprite(playerElement, playerDirection, true);
        return;
      } catch (err) {
  ...
  function startDialogue(npcElement) {
    ...
    try {
      ...
      if (window.currentNpcs && npcId) {
        const npcData = window.currentNpcs.find(npc => npc.id.toString() === npcId);
    ...
    const dialogueBox = document.getElementById('dialogue-box') || createSimpleDialogueBox();
    ...
    const nameElement = dialogueBox.querySelector('#dialogue-name') || dialogueBox.querySelector('.dialogue-name');
    if (nameElement) {
      nameElement.textContent = npcName;
    }
    
    const textElement = dialogueBox.querySelector('#dialogue-text') || dialogueBox.querySelector('.dialogue-text');
    if (textElement) {
      textElement.textContent = 'Hello there! I cannot talk much due to technical difficulties.';
    }
  }
